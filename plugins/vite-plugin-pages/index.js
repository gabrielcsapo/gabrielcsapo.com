...
async function getRoutesFromDir(dir, baseDir, projectDir) {
  ...
  const dirPromises = entries.map(async (entry) => {
    if (entry.isDirectory()) {
      return getRoutesFromDir(path.join(dir, entry.name), baseDir, projectDir);
    } else if (entry.name.endsWith(".jsx")) {
      const routePath = getSlug(path.resolve(dir, entry.name).replace(baseDir + "/pages", ""));
      // we are going to find the closet layout or use the default layout
      const layout =
        entries.find((e) => {
          e.name === "Layout.jsx";
        }) ?? path.resolve(projectDir, "src/Layout.jsx");
    ...
    async load(id) {
      if (id === resolvedVirtualModuleId) {
        ...
        if (fse.existsSync(potentialTopLevelFilesDirectory)) {
          fse.copySync(potentialTopLevelFilesDirectory, path.resolve(baseDir, "public", "files"));
        }

        const hashes = {};
        const posts = findAllBlogPosts(postsDir);
        const generatedPostRoutes = posts.map((post) => {
          const potentialFilesDirectory = path.resolve(path.dirname(post.locationOnDisk), "files");
          // TODO: this should really be a part of the parsing of the markdown document
          if (fse.existsSync(potentialFilesDirectory)) {
            fse.copySync(potentialFilesDirectory, path.resolve(baseDir, "public", "files"));
          }
